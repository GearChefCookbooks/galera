...
mysql_tarball = node["galera"]["wsrep_tarball"][node['kernel']['machine']]
# strip .tar.gz
mysql_package = mysql_tarball[0..-8]

mysql_wsrep_source = node['galera']['wsrep_source']
galera_source = node['galera']['source']
...
when 'centos', 'redhat', 'fedora', 'suse', 'scientific', 'amazon'
  galera_package = node['galera']['package'][node['kernel']['machine']]['rpm']
else
  galera_package = node['galera']['package'][node['kernel']['machine']]['deb']
end
